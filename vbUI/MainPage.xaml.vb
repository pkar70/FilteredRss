...
Public NotInheritable Class MainPage
    ...
    Public Async Function ShowTorrentData(oItem As VBlib.JedenItem) As Task
        ...
        If oItem.sLinkToDescr.Contains("ekai.pl") Then
            ...
            If iInd > 1 Then
                temppage = temppage.Substring(iInd)
                iInd = temppage.IndexOf("</article>")
                temppage = temppage.Substring(0, iInd + "</article>".Length)
        ...
        If oItem.sLinkToDescr.Contains("lovekrakow.pl") Then
            ...
            ' Await VBlib.HttpPageAsync(oItem.sLinkToDescr)
            Dim iInd = temppage.IndexOf("<article id=""article")
            If iInd > 1 Then
                temppage = temppage.Substring(iInd)
                iInd = temppage.IndexOf("<div id=""right_column")
                iInd = temppage.LastIndexOf("</article>", iInd)
                temppage = temppage.Substring(0, iInd + "</article>".Length)
    ...
    Public Sub ShowTorrentData(sGuid As String)
        ...
        If VBlib.App.glItems Is Nothing Then
            Me.MsgBox("IMPOSSIBLE, ShowTorrentData and oAllItems null")
            Exit Sub
        End If
        ...
        Me.MsgBox("IMPOSSIBLE? Cannot find such item")
        Exit Sub
...
#If DEBUG Then
        ...
        ' bo tylko u mnie, i tylko gdy coś faktycznie jest, bo inaczej nie ma sensu
        If VBlib.App.glItems IsNot Nothing AndAlso VBlib.App.glItems.Count > 1 AndAlso Not Await VBlib.DialogBoxYNAsync("Usunąć te które mają obrazek http 404?") Then Return
